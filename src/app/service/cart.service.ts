...
export class CartService {
  private behaviorSubject: BehaviorSubject<Cart> = new BehaviorSubject<Cart>(
    new Cart(),
  );
  ...
  addCartProduct(cartProduct: CartProduct): void {
    // Get current values.
    const cart: Cart = this.behaviorSubject.getValue();
    ...
    if (this.itemExistsInCart(cartProduct)) {
      ...
      this.behaviorSubject.next(cart);
    }
  }
  ...
  clearCart(): void {
    this.behaviorSubject.next(new Cart());
  }
}
